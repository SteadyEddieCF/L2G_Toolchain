...
test('v3.10.1 rich governed records preserve blocker, dependency, service, responsibility, request, follow-up, and lineage fields',async({page})=>{
  ...
  const result=await page.evaluate(()=>{
    ...
    const followup={followup_id:'followup-rich-001',ownership_record_id:'ownership-rich-001',request_id:'request-rich-001',practice_id:'AC.L2-3.1.1',objective_id:'AC.L2-3.1.1[a]',provider:'Synthetic Cloud Provider',notes:'Confirm export delivery',state:'Pending',owner:'Synthetic Advisor',due_date:'2026-10-08',contract_validation_required:true,access_limitation:'Client tenant export required.',source_fingerprint:'sha256:'+'d'.repeat(64),action_id:'action-rich-001'};
  ...
  expect(result.f.source_record).toMatchObject({followup_id:'followup-rich-001',request_id:'request-rich-001',provider:'Synthetic Cloud Provider',topic:'Confirm export delivery',action_id:'action-rich-001'});
