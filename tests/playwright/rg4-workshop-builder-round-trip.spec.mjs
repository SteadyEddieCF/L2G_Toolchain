...
const EXPECTED = {
  ...
  workshopRuntime: 'a1f63944d0573587e2a5b7826f72befa16f6d89b849f3129f7f6dbb080da54ca',
  builderRuntime: '96ecb1caee5f7ba278c3b46c666d703423e2db40cac22f8431e70485e5d76a17',
  sspRuntime: 'bfd9d10a780809ba259406f0770641da6a40ac2d8a6d1e372b070d6f5273351b'
};
...
test('RG-4 Workshop v79 and Builder/Merger v3.10 round trip produces exact evidence and records blockers', async ({ browser }, testInfo) => {
  ...
  await workshop.setViewportSize({ width: 1024, height: 720 });
  ...
  await builder.setViewportSize({ width: 1024, height: 720 });
  ...
  const extraction = await builder.evaluate(async ({ workbookB64, handoffText }) => {
    ...
    const workbook = new File([decode(workbookB64)], 'RG4_BuilderMerger_v3.10_Generated_Workbook.xlsx', {
    ...
    return {
      ...
      rows: {
        practiceCandidates: state.reviewWorkbookRows.practiceCandidates.length,
        objectiveCandidates: state.reviewWorkbookRows.objectiveCandidates.length,
        advisorCandidates: state.reviewWorkbookRows.advisorReviewCandidates.length,
        allCandidates: state.reviewWorkshopRows.allResults.length
      },
  ...
  const mergeResult = await workshopReturn.evaluate(async (text) => {
    ...
    const substantive = () => ({
      practices: clone(state.practices),
      objectives: clone(state.objectiveReviews),
      decisions: clone(state.decisions),
      actions: clone(state.actionRegister.actions),
      ownership: clone(state.evidenceOwnershipV77.accepted_records),
      requests: clone(state.evidenceOwnershipV77.requests),
      followups: clone(state.evidenceOwnershipV77.provider_followups),
    ...
    const mismatchedIdentity = JSON.parse(text);
    mismatchedIdentity.practice_results[0].Practice_ID = mismatchedIdentity.practice_results[1].Practice_ID;
    ...
    const mismatch = v57PreviewMergeText(JSON.stringify(mismatchedIdentity), 'mismatched-practice-identity');
    ...
    return {
      ...
      unmatchedPractices: preview.unmatched_practices,
      unmatchedObjectives: preview.unmatched_objectives,
      historyCount: state.workbookMergeV57.history.length,
      duplicateBlocked: second.duplicate === true,
      unrelatedUnchanged: {
      ...
      negative: {
        ...
        mismatchedIdentityAccepted: !mismatch.blocking,
        mismatchedIdentityUnmatchedPractices: mismatch.unmatched_practices,
  ...
  const blockerReport = {
    ...
    blockers: [
      {
        id: 'WKS-RG4-001',
        condition: 'Frozen registry declares Workbook Handoff 1.7
but exact Workshop v79 output declares package_version 1.0 and a separate enhancement version 1.7.',
        expected: 'package_version 1.7',
        actual: handoff.package_version,
        enhancement: handoff.handoff_schema_enhancements_version
      },
      {
        id: 'WKS-RG4-002',
        condition: 'Workshop v79 accepts unknown Workbook Merge versions as trusted/nonblocking.',
        actual: mergeResult.negative.unknownVersionAccepted
      },
      {
        id: 'WKS-RG4-003',
        condition: 'Workshop v79 accepts unknown top-level Workbook Merge properties.',
        actual: mergeResult.negative.extraPropertyAccepted
      },
      {
        id: 'WKS-RG4-004',
        condition: 'Workshop v79 does not explicitly reject duplicate JSON keys when the final value is valid.',
        actual: mergeResult.negative.duplicateKeyAccepted
      },
      {
        id: 'WKS-RG4-005',
        condition: 'Workshop v79 accepts a duplicated/mismatched practice identity as trusted and nonblocking.',
        actual: { accepted: mergeResult.negative.mismatchedIdentityAccepted, unmatched: mergeResult.negative.mismatchedIdentityUnmatchedPractices }
      },
      {
        id: 'RG4-ROUNDTRIP-006',
        condition: 'Builder/Merger workbook does not retain exact Workshop action/ownership IDs or action owner/due date/blocker/access-limitation fields required by issue #101.',
        actual: extraction.tokenPresence
      }
    ],
    passing: {
      merge_package_version: extraction.package.package_version,
      practice_ids_preserved: true,
      objective_ids_preserved_after_canonicalization: true,
      formula_cells_preserved: extraction.formulaCount,
      preview_non_mutating: mergeResult.previewNonMutating,
      duplicate_reimport_blocked: mergeResult.duplicateBlocked,
      injected_text_inert: !mergeResult.negative.injectedTextExecuted
    }
  };
...
test('RG-4 Workshop v79 and SSP v1.9.17 Handoff/Return 1.0 remain isolated from SSP-owned RG-4 evidence history', async ({ browser }, testInfo) => {
  ...
  const route = await ssp.evaluate(async ({ handoff, sidecarText, docxB64 }) => {
    ...
    return {
      ...
      validatedKind: validated.package_kind,
      validatedVersion: validated.package_version,
  ...
  const downloadPromise = ssp.waitForEvent('download');
  await ssp.evaluate(() => __sspTestHooks.l2gExportReturnPackage());
  const returnDownload = await downloadPromise;
  ...
  const previewResult = await workshopReturn.evaluate((pkg) => {
    ...
    return { before, after, errors: preview.errors, warnings: preview.warnings, controls: preview.controls.length };
  }, returnPackage);
  ...
  await testInfo.attach('RG4_Workshop_SSP_Route_ResumšœÛÛ‰ËÂˆ›ÙNˆY™™\‹™œ›ÛJ”ÓÓ‹œİš[™ÚYJÂˆÛÜšÜÚÜİ×ÜÜÜˆÈXÚØYÙWİ™\œÚ[Ûˆ[™Ù™‹œXÚØYÙWİ™\œÚ[Û‹ÛÛ›ÛÎˆ[™Ù™‹˜ÛÛ›ÛË›[™İ™ÍÚ\İÜWİ[˜Ú[™ÙYˆYHKˆÜÜİ×İÛÜšÜÚÜˆÈXÚØYÙWİ™\œÚ[Ûˆ™]\›”XÚØYÙKœXÚØYÙWİ™\œÚ[Û‹ÛÛ›ÛÎˆ™]\›”XÚØYÙK˜ÛÛ›ÛË›[™İ™]šY]×Û›Û—Û]]][™ÎˆYK™ÍÚ\İÜWØÛÜYYˆ˜[ÙHBˆK[ŠJKˆÛÛ[\Nˆ	Ø\XØ][Û‹ÚœÛÛ‰ÂˆJNÂ‚ˆ›Üˆ
ÛÛœİ\œ›ÜœÈÙˆİÛÜšÜÚÜ\œ›ÜœËÜÜ\œ›ÜœË™]\›‘\œ›Üœ×JHÂˆ^Xİ
\œ›ÜœË™^\›˜[™\]Y\İÊKÑ\]X[
×JNÂˆ^Xİ
\œ›ÜœËœYÙQ\œ›ÜœÊKÑ\]X[
×JNÂˆ^Xİ
\œ›ÜœË˜ÛÛœÛÛQ\œ›ÜœÊKÑ\]X[
×JNÂˆBˆ]ØZ]›ÛZ\ÙK˜[
İÛÜšÜÚÜ˜ÛÜÙJ
KÜÜ˜ÛÜÙJ
KÛÜšÜÚÜ™]\›‹˜ÛÜÙJ
WJNÂˆœËœ›TŞ[˜Ê[\È™Xİ\œÚ]™NˆYK›Ü˜ÙNˆYHJNÂŸJNÂ