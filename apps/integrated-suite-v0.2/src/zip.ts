namespace L2G {
  ...
  export function createStoredZip(inputEntries: ZipEntry[]): Uint8Array {
    ...
    for (const entry of entries) {
      ...
      if (entry.data.length > ARCHIVE_LIMITS.maxEntryBytes) throw new Error(`Archive entry exceeds limit: ${entry.path}`);
      expanded += entry.data.length;
      if (expanded > ARCHIVE_LIMITS.maxExpandedBytes) throw new Error("Archive expanded size exceeds limit.");
    }
  ...
  export function readStoredZip(data: Uint8Array): ZipEntry[] {
    if (data.length < 22) throw new Error("Archive is too small.");
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    let endOffset = -1;
    for (let offset = data.length - 22; offset >= Math.max(0, data.length - 65557); offset -= 1) {
      if (view.getUint32(offset, true) === END_SIGNATURE) { endOffset = offset; break; }
    }
    if (endOffset < 0) throw new Error("ZIP end record missing.");
    const entryCount = view.getUint16(endOffset + 10, true);
    const centralSize = view.getUint32(endOffset + 12, true);
    const centralOffset = view.getUint32(endOffset + 16, true);
    const commentLength = view.getUint16(endOffset + 20, true);
    if (endOffset + 22 + commentLength !== data.length) throw new Error("Unexpected trailing archive content.");
    if (entryCount > ARCHIVE_LIMITS.maxEntries || centralOffset + centralSize > endOffset) throw new Error("Archive directory is invalid.");
    const paths = new Set<string>();
    const output: ZipEntry[] = [];
    let cursor = centralOffset;
    let expanded = 0;
    for (let index = 0; index < entryCount; index += 1) {
      if (cursor + 46 > data.length || view.getUint32(cursor, true) !== CENTRAL_SIGNATURE) throw new Error("Invalid central directory.");
      ...
      if ((flags & 1) !== 0 || method !== 0 || compressedSize !== payloadSize) throw new Error("Compressed or encrypted ZIP entries are rejected.");
      const nameStart = cursor + 46;
      const nameEnd = nameStart + nameLength;
      if (nameEnd + extraLength + fileCommentLength > data.length) throw new Error("Truncated central directory.");
      ...
      if (expanded > ARCHIVE_LIMITS.maxExpandedBytes || payloadSize > ARCHIVE_LIMITS.maxEntryBytes) throw new Error("Archive payload exceeds limit.");
      if (localOffset + 30 > data.length || view.getUint32(localOffset, true) !== LOCAL_SIGNATURE) throw new Error(`Invalid local header: ${path}`);
      const localNameLength = view.getUint16(localOffset + 26, true);
      const localExtraLength = view.getUint16(localOffset + 28, true);
      const localNameStart = localOffset + 30;
      const localNameEnd = localNameStart + localNameLength;
      const localPath = decodeUtf8(data.subarray(localNameStart, localNameEnd));
      if (localPath !== path) throw new Error(`Header path mismatch: ${path}`);
      const payloadStart = localNameEnd + localExtraLength;
      const payloadEnd = payloadStart + compressedSize;
      if (payloadEnd > data.length) throw new Error(`Truncated payload: ${path}`);
      const payload = data.slice(payloadStart, payloadEnd);
      if (crc32(payload) !== expectedCrc) throw new Error(`CRC mismatch: ${path}`);
