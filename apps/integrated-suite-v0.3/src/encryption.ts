namespace L2G {
  ...
  export async function deriveAesKey(baseKey: CryptoKey, salt: Uint8Array): Promise<CryptoKey> {
    return crypto.subtle.deriveKey({ name: "PBKDF2", hash: "SHA-256", salt: salt.slice().buffer, iterations: 600000 }, baseKey, { name: "AES-GCM", length: 256 }, false, ["encrypt", "decrypt"]);
  }

  export async function encryptProject(document: ProjectDocument, baseKey: CryptoKey, purpose: EnvelopePurpose, options?: { salt?: Uint8Array; iv?: Uint8Array }): Promise<{ bytes: Uint8Array; metadata: EnvelopeMetadata; key: CryptoKey; salt: Uint8Array }> {
    ...
    const metadata: EnvelopeMetadata = {
      kind: "l2g_encrypted_project_v1", version: "1.0", purpose,
      ...
      inner: { media_type: "application/vnd.l2g.project+zip", project_kind: "l2g_project_v1", schema_version: "1.0", plaintext_bytes: inner.length, plaintext_sha256: await sha256Hex(inner) },
    ...
    const ciphertext = new Uint8Array(await crypto.subtle.encrypt({ name: "AES-GCM", iv: iv.slice().buffer, additionalData: additionalData.slice().buffer, tagLength: 128 }, key, inner.slice().buffer));
    const bytes = createStoredZip([{ path: "ciphertext.bin", data: ciphertext }, { path: "envelope.json", data: utf8(stableStringify(metadata)) }]);
    if (bytes.length > ENCRYPTED_LIMITS.maxOuterBytes) throw new Error("Encrypted project exceeds the v0.3 size limit.");
    return { bytes, metadata, key, salt };
  }

  export function isEncryptedPackage(bytes: Uint8Array): boolean {
    try { const names = readStoredZip(bytes).map(entry => entry.path).sort(); return names.length === 2 && names.every((path, index) => path === OUTER_PATHS[index]); }
    catch { return false; }
  }

  export async function decryptProject(bytes: Uint8Array, passphrase: string, expectedPurpose: EnvelopePurpose): Promise<{ document: ProjectDocument; baseKey: CryptoKey; key: CryptoKey; salt: Uint8Array; metadata: EnvelopeMetadata; migrated: boolean }> {
    if (bytes.length > ENCRYPTED_LIMITS.maxOuterBytes) throw new Error("Encrypted project exceeds the v0.3 size limit.");
    ...
    try { plaintext = new Uint8Array(await crypto.subtle.decrypt({ name: "AES-GCM", iv: iv.slice().buffer, additionalData: additionalData.slice().buffer, tagLength: 128 }, key, ciphertext.slice().buffer)); }
    catch { throw new Error("The passphrase is incorrect or the encrypted content was modified."); }
    if (plaintext.length !== metadata.inner.plaintext_bytes || await sha256Hex(plaintext) !== metadata.inner.plaintext_sha256) throw new Error("The passphrase is incorrect or the encrypted content was modified.");
    const result = await deserializeInnerProject(plaintext, expectedPurpose === "portable-project");
    return { document: result.document, baseKey, key, salt, metadata, migrated: result.legacy };
  }
  ...
  export function validateEnvelopeMetadata(value: unknown, expectedPurpose: EnvelopePurpose): asserts value is EnvelopeMetadata {
    ...
    const cipher = value.cipher; const kdf = value.kdf; const inner = value.inner; const application = value.application;
    ...
    b64ToBytes(cipher.iv_b64, 12); b64ToBytes(kdf.salt_b64, 16);
  }
}
