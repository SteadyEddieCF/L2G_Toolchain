namespace L2G {
  export class RecoveryStore {
    private readonly dbName = "l2g-integrated-suite-v0-3";
    ...
    async save(bytes: Uint8Array): Promise<void> {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        transaction.objectStore(this.storeName).put({ kind: "l2g_encrypted_recovery_v1", saved_at: nowIso(), bytes: bytes.slice().buffer } satisfies RecoveryRecord, this.key);
  ...
  interface CommandSnapshot { description: string; action: string; object_type: string; object_id: string; before: ProjectState; after: ProjectState; event_id: string; }
  ...
  export class ProjectStore {
    ...
    constructor(document: ProjectDocument) { validateProjectDocument(document, true); this.documentValue = deepClone(document); }
    ...
    execute(description: string, action: string, objectType: string, objectId: string, mutator: (state: ProjectState) => void): void {
      const before = deepClone(this.documentValue.state);
      const after = deepClone(before);
      mutator(after);
      const candidate = deepClone(this.documentValue);
      candidate.state = after;
      validateProjectDocument(candidate, true);
      if (stableStringify(before, 0) === stableStringify(after, 0)) return;
      this.documentValue.state = after;
      touchProject(this.documentValue);
      const event = appendHistory(this.documentValue, action, objectType, objectId, description);
      this.undoStack.push({ description, action, object_type: objectType, object_id: objectId, before, after: deepClone(after), event_id: event.event_id });
    ...
    undo(): void {
      ...
      touchProject(this.documentValue);
      appendHistory(this.documentValue, "command.undone", snapshot.object_type, snapshot.object_id, `Undid “${snapshot.description}”.`, snapshot.event_id);
    ...
    redo(): void {
      ...
      touchProject(this.documentValue);
      appendHistory(this.documentValue, "command.redone", snapshot.object_type, snapshot.object_id, `Redid “${snapshot.description}”.`);
      this.undoStack.push(snapshot);
      this.notify();
    }

    createCheckpoint(name: string): Checkpoint {
      const checkpoint: Checkpoint = { checkpoint_id: newId("checkpoint"), name: sanitizePlainText(name || "Manual checkpoint", 120), created_at: nowIso(), state: deepClone(this.documentValue.state) };
    ...
    restoreCheckpoint(checkpointId: string): void {
      ...
      this.undoStack.push({ description: `Restore checkpoint “${checkpoint.name}”`, action: "checkpoint.restored", object_type: "checkpoint", object_id: checkpointId, before, after, event_id: event.event_id });
      this.redoStack = [];
      this.notify();
    }
    ...
    private applyDataState(source: ProjectState): void {
      ...
      validateProjectDocument(this.documentValue, true);
    }
