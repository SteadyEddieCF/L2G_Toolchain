from __future__ import annotations

import base64
import hashlib
import json
import os
import shutil
import subprocess
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
REPO = ROOT.parents[1]
BUILD = ROOT / "build"
DIST = ROOT / "dist"
RELEASE = json.loads((ROOT / "release" / "release.json").read_text(encoding="utf-8"))
REGISTRY_PATH = REPO / "apps" / "integrated-suite-v0.2" / "contracts" / "registry.json"
REGISTRY = json.loads(REGISTRY_PATH.read_text(encoding="utf-8"))


def normalize(text: str) -> str:
    return text.replace("\r\n", "\n").replace("\r", "\n").rstrip() + "\n"


def sha256(data: bytes) -> str:
    return hashlib.sha256(data).hexdigest()


def compile_typescript() -> None:
    local = ROOT / "node_modules" / ".bin" / ("tsc.cmd" if os.name == "nt" else "tsc")
    compiler = str(local) if local.exists() else shutil.which("tsc")
    if not compiler:
        raise SystemExit("TypeScript compiler is unavailable. Run npm ci in apps/integrated-suite-v0.3.")
    BUILD.mkdir(exist_ok=True)
    subprocess.run([compiler, "-p", str(ROOT / "tsconfig.build.json")], cwd=ROOT, check=True)


def create_sbom(artifact_name: str, artifact_hash: str) -> dict:
    namespace = f"https://l2g.local/spdx/integrated-suite-v0.3.0/{artifact_hash}"
    return {
        "spdxVersion": "SPDX-2.3",
        "dataLicense": "CC0-1.0",
        "SPDXID": "SPDXRef-DOCUMENT",
        "name": "L2G Integrated Suite v0.3.0",
        "documentNamespace": namespace,
        "creationInfo": {
            "created": "2026-08-04T00:00:00Z",
            "creators": ["Tool: L2G deterministic build.py"]
        },
        "packages": [
            {
                "name": "L2G Integrated Suite Engagement Spine",
                "SPDXID": "SPDXRef-Package-App",
                "versionInfo": "0.3.0",
                "downloadLocation": "NOASSERTION",
                "filesAnalyzed": True,
                "checksums": [{"algorithm": "SHA256", "checksumValue": artifact_hash}],
                "licenseConcluded": "NOASSERTION",
                "licenseDeclared": "NOASSERTION",
                "copyrightText": "NOASSERTION"
            },
            {
                "name": "TypeScript",
                "SPDXID": "SPDXRef-Package-TypeScript",
                "versionInfo": "5.8.3",
                "downloadLocation": "https://registry.npmjs.org/typescript/-/typescript-5.8.3.tgz",
                "filesAnalyzed": False,
                "licenseConcluded": "Apache-2.0",
                "licenseDeclared": "Apache-2.0",
                "copyrightText": "NOASSERTION"
            }
        ],
        "relationships": [
            {"spdxElementId": "SPDXRef-DOCUMENT", "relationshipType": "DESCRIBES", "relatedSpdxElement": "SPDXRef-Package-App"},
            {"spdxElementId": "SPDXRef-Package-App", "relationshipType": "BUILD_DEPENDENCY_OF", "relatedSpdxElement": "SPDXRef-Package-TypeScript"}
        ]
    }


def main() -> None:
    DIST.mkdir(exist_ok=True)
    compile_typescript()
    js = normalize((BUILD / "app.js").read_text(encoding="utf-8"))
    css = normalize((ROOT / "src" / "styles.css").read_text(encoding="utf-8"))
    release_json = json.dumps(RELEASE, sort_keys=True, separators=(",", ":")).replace("<", "\\u003c")
    registry_json = json.dumps(REGISTRY, sort_keys=True, separators=(",", ":")).replace("<", "\\u003c")
    script = normalize(
        f"window.__L2G_RELEASE__=Object.freeze({release_json});\n"
        f"window.__L2G_CONTRACT_REGISTRY__=Object.freeze({registry_json});\n"
        f"{js}"
    )
    script_hash = base64.b64encode(hashlib.sha256(script.encode("utf-8")).digest()).decode("ascii")
    style_hash = base64.b64encode(hashlib.sha256(css.encode("utf-8")).digest()).decode("ascii")
    csp = (
        "default-src 'none'; "
        f"script-src 'sha256-{script_hash}'; "
        f"style-src 'sha256-{style_hash}'; "
        "connect-src 'none'; img-src data:; font-src 'none'; object-src 'none'; "
        "frame-src 'none'; child-src blob:; worker-src blob:; form-action 'none'; "
        "base-uri 'none'; media-src 'none'"
    )
    html = normalize((ROOT / "src" / "template.html").read_text(encoding="utf-8"))
    html = html.replace("__CSP__", csp).replace("__CSS__", css).replace("__JS__", script)
    artifact = DIST / RELEASE["artifact_name"]
    artifact.write_text(html, encoding="utf-8", newline="\n")
    artifact_hash = sha256(artifact.read_bytes())
    manifest = {
        "kind": "l2g_integrated_suite_release_v1",
        "application": RELEASE["application"],
        "version": RELEASE["version"],
        "artifact": artifact.name,
        "sha256": artifact_hash,
        "size": artifact.stat().st_size,
        "envelope_kind": RELEASE["envelope_kind"],
        "project_kind": RELEASE["project_kind"],
        "engagement_schema_kind": RELEASE["engagement_schema_kind"],
        "engagement_schema_version": RELEASE["engagement_schema_version"],
        "content_security_policy": csp,
        "product_runtime_compatibility_baseline": RELEASE["product_runtime_compatibility_baseline"],
        "synthetic_only": True,
        "production_data_authorized": False,
        "runtime_network_dependencies": 0,
        "inherited_validated_primitives": [
            "apps/integrated-suite-v0.2/src/util.ts",
            "apps/integrated-suite-v0.2/src/zip.ts"
        ]
    }
    (DIST / "release-manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8", newline="\n")
    sbom = create_sbom(artifact.name, artifact_hash)
    (DIST / "sbom.spdx.json").write_text(json.dumps(sbom, indent=2, sort_keys=True) + "\n", encoding="utf-8", newline="\n")
    output = ROOT / "releases" / "v0.3.0"
    shutil.rmtree(output, ignore_errors=True)
    output.mkdir(parents=True)
    shutil.copy2(artifact, output / artifact.name)
    shutil.copy2(DIST / "release-manifest.json", output / "release-manifest.json")
    shutil.copy2(DIST / "sbom.spdx.json", output / "sbom.spdx.json")
    shutil.copy2(ROOT / "release" / "RELEASE_NOTES_v0.3.0.md", output / "RELEASE_NOTES.md")
    names = [artifact.name, "release-manifest.json", "sbom.spdx.json", "RELEASE_NOTES.md"]
    checksums = [f"{sha256((output / name).read_bytes())}  {name}" for name in sorted(names)]
    (output / "SHA256SUMS.txt").write_text("\n".join(checksums) + "\n", encoding="utf-8", newline="\n")
    print(json.dumps(manifest, sort_keys=True))


if __name__ == "__main__":
    main()
