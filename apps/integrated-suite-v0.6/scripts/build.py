...
def compile_typescript() -> None:
    ...
    if not compiler:
        raise SystemExit("TypeScript compiler is unavailable. Run npm ci in apps/integrated-suite-v0.6.")
...
style = "\n".join([
    (BASE / "src" / "styles.css").read_text(encoding="utf-8").strip(),
    (ROOT / "src" / "scope.css").read_text(encoding="utf-8").strip(),
])
...
remaining = [token for token in ["__L2G_STYLE__", "__L2G_CSP__", "__L2G_RELEASE_JSON__", "__L2G_REGISTRY_JSON__", "__L2G_SCRIPT__"] if token in html]
if remaining:
...
manifest = {
    ...
    "scope_schema_kind": RELEASE["scope_schema_kind"],
    "scope_schema_version": RELEASE["scope_schema_version"],
    ...
    "inherited_validated_primitives": [
        ...
        "apps/integrated-suite-v0.5/src/engagement.ts",
        "apps/integrated-suite-v0.5/src/evidence.ts",
        "apps/integrated-suite-v0.5/src/pre-engagement.ts",
        "apps/integrated-suite-v0.5/src/interviews.ts",
        "apps/integrated-suite-v0.5/src/project.ts",
    ],
}
...
schema_files = [
    ...
    BASE / "schemas" / "l2g_pre_engagement_v1.schema.json",
    BASE / "schemas" / "l2g_interview_sessions_v1.schema.json",
...
sha_lines = [f"{hashlib.sha256(file.read_bytes()).hexdigest()}  {file.name}" for file in sorted(RELEASE_DIR.iterdir()) if file.name != "SHA256SUMS.txt"]
