...
def write(name,value): (FIXTURES/name).write_text(json.dumps(value,indent=2,ensure_ascii=False)+"\n",encoding="utf-8")
...
raw=json.dumps(base,separators=(",",":"),ensure_ascii=False).replace('"schema_version":"1.0"','"schema_version":"1.0","schema_version":"1.0"',1);(FIXTURES/"duplicate_key_extension.json.txt").write_text(raw+"\n",encoding="utf-8")
manifest={}
...
(FIXTURES/"FIXTURE_SHA256.json").write_text(json.dumps(manifest,indent=2)+"\n",encoding="utf-8")
