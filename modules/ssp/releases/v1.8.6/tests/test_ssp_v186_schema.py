...
valid=json.loads((FIX/'Portfolio_CRM_McFirecoal_v1.8.6.json').read_text())
errors=list(validator.iter_errors(valid)); assert not errors,[e.message for e in errors[:10]]
assert valid['schemaVersion']=='1.6.0'
assert len(valid['modules'])==4 and len(valid['moduleRequirements'])==440
for key in ['reviewRoleAssignments','reviewSubmissions','reviewDispositions','approvalRecords']:
...
module=json.loads((FIX/'Module_Exchange_Product_Alpha_v1.8.6.json').read_text())
...
result={'release':'v1.8.6','suite':'schema-formal-review-regression','status':'passed','validFixture':{'modules':len(valid['modules']),'moduleRequirements':len(valid['moduleRequirements'])},'invalidFixtures':invalid_results}
(ROOT/'CMMC_L2_SSP_v1.8.6_Schema_Regression.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
