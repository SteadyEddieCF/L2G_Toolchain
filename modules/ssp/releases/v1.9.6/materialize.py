...
PATCH_PARTS = sorted((ROOT / "source").glob("runtime-v1.9.5.1-to-v1.9.6.patch.part*"))
...
if len(PATCH_PARTS) != EXPECTED_PART_COUNT:
    raise SystemExit(f"expected {EXPECTED_PART_COUNT} patch parts, found {len(PATCH_PARTS)}")
baseline_bytes = BASELINE.read_bytes()
require("runtime-source baseline", digest(baseline_bytes), EXPECTED_BASELINE)
patch_bytes = b"".join(part.read_bytes() for part in PATCH_PARTS)
require("unified patch", digest(patch_bytes), EXPECTED_PATCH)
runtime = apply_unified_diff(
    baseline_bytes.decode("utf-8"), patch_bytes.decode("utf-8")
).encode("utf-8")
