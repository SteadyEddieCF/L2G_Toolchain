#!/usr/bin/env python3
"""Materialize the exact L2G Builder/Merger v3.10.1 runtime from v3.10, the reviewed v3.10.1 patch, and the issue #114 corrections."""
...
PRIMARY_CORRECTED_SHA256='b45e0d41df20920be383fc6aa4a88e8f25feb9c564296c0e14b8c81a64bb115f'
...
def main()->int:
 ...
 if a.verify_only:data=out.read_bytes()
  ...
  primary=sorted(p for p in source.glob('workbook_merge_governance_correction.patch.*') if p.name!='workbook_merge_governance_correction.patch.011')
  linkage=source/'workbook_merge_governance_correction.patch.011'
  if not primary:raise SystemExit('primary issue #114 correction patch parts missing')
  if not linkage.is_file():raise SystemExit('source-Handoff linkage correction patch missing')
  primary_corrected=apply_patch(previous.decode('utf-8'),''.join(p.read_text('utf-8') for p in primary)).encode('utf-8')
  if sha(primary_corrected)!=PRIMARY_CORRECTED_SHA256:raise SystemExit(f'primary corrected candidate identity mismatch: {sha(primary_corrected)}')
  data=apply_patch(primary_corrected.decode('utf-8'),linkage.read_text('utf-8')).encode('utf-8');out.parent.mkdir(parents=True,exist_ok=True);out.write_bytes(data)
 actual=sha(data)
